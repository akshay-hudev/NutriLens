...
Preliminary results indicate that sparse-view geometric reasoning reduces volume
and calorie error relative to 2D-area baselines, primarily for non-planar foods.
Exact values are computed by the evaluation pipeline and are not hardcoded in
this manuscript. The system remains sensitive
...
Table~\ref{tab:main_results} summarises pilot results. The sparse-view proxy
consistently yields the lowest volume and calorie error among the evaluated
baselines in our pilot set. This improvement is attributable to the proxy
height estimate: for mounded or layered foods, footprint-only methods
systematically underestimate volume, and the additional viewpoint cues
constrain the vertical extent. Fig.~\ref{fig:baseline} visualises the
comparison using evaluation outputs.

\begin{table}[t]
\centering
\caption{Pilot comparison of portion estimators. Numeric values are generated
from the evaluation pipeline (see evaluation/outputs) and are intentionally
omitted here to avoid hardcoded results.}
...
is suitable for interactive prototypes, while optional GPU-accelerated
COLMAP paths increase pose quality at the cost of longer runtimes.

\begin{table}[t]
\centering
\caption{Pipeline runtime summary. Numeric values are produced by the
evaluation pipeline and are not hardcoded here.}
\label{tab:runtime}
...
\begin{table}[t]
\centering
\caption{Effect of input view count on volume error and runtime.
Numeric values are generated by the evaluation pipeline.}
\label{tab:views}
\renewcommand{\arraystretch}{1.12}
\begin{tabular}{lllcc}
\toprule
...
\textbf{View count.} Increasing view count reduces volume error, with the
largest gains typically occurring between one and two views. Improvements
from additional views diminish beyond three views, suggesting that scale
uncertainty and mask quality dominate residual error.

\textbf{Segmentation quality.} Cleaner segmentation masks improve volume
estimates substantially, confirming that segmentation quality is a critical
bottleneck. This motivates future integration of food-specific segmentation
models.

\textbf{Reconstruction proxy size.} Increasing the number of proxy primitives
improves visual smoothness and modestly improves volume consistency for
complex shapes, but increases runtime. A moderate primitive count provides
the best tradeoff for interactive use.

\textbf{Scale sensitivity.} Perturbing the assumed plate diameter shifts the
volume estimate substantially (cubic scaling per Eq.~\eqref{eq:scalecube}),
confirming that scale is the dominant uncertainty source when no metric
anchor is available.
...
Pilot results on controlled food sessions indicate that adding sparse-view
geometric cues reduces volume and calorie error relative to a 2D mask-area
baseline, with the largest gains on mounded, layered, and geometrically
irregular foods. The system
